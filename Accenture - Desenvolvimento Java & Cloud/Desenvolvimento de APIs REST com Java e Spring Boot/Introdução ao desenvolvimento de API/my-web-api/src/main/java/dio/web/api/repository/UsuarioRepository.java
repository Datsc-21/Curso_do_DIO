package dio.web.api.repository;

import dio.web.api.model.Usuario;
import org.springframework.stereotype.Repository;

import java.util.ArrayList;
import java.util.List;

@Repository
public class UsuarioRepository {
  public void save(Usuario usuario){
      if(usuario.getId()==null)
          System.out.println("SAVE - recebendo o usuário na camada de repositorio");
      else
          System.out.println("UPDATE - recebendo o usuário na camada de repositorio");
  }
  public void deleteById(Integer id){
      System.out.println(String.format("DELETE - recebendo o id: %d para excluir um usuário", id));
      System.out.println(id);
  }
    public List<Usuario> findAll(){
      System.out.println("LIST - listando os usuários do sitema");
      List<Usuario> usuarios = new ArrayList<>();
      usuarios.add(new Usuario("Danny", "password"));
      usuarios.add(new Usuario("Frank", "Masterpass"));
      return usuarios;
    }
    public Usuario findById(Integer id){
      System.out.println(String.format("FIND/id - Recebendo o id: %d para localizar um usuário", id));
      return new Usuario("Danny", "password");
    }
    public Usuario findByUsername(String usernamer){
      System.out.println(String.format("FIND/username - recebendo o username: %s para localizar um usuário", usernamer));
      return new Usuario("Danny","password");
    }
}
